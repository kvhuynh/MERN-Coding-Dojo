...
const pokemonIdDivisibleBy3 = pokemon.filter(p => p.id % 3 === 0);
...
const pokemonIdGreaterThan89 = pokemon.filter(p => p.id > 89).map(p => p.name);
console.log(pokemonIdGreaterThan89);

const poisonTypeNames = pokemon.filter(p => p.types.length === 1 && p.types.includes("poison")).map(p => p.name);
console.log(poisonTypeNames);

const secondTypeFlying = pokemon.filter(p => p.types.length === 2 && p.types[1] === "flying").map(p => p.name + " " + p.types[0]);
console.log(secondTypeFlying);

const normalTypeCount = pokemon.filter(p => p.types.includes("normal") && p.types.length === 1).length
console.log("normal type count is: " + normalTypeCount);
